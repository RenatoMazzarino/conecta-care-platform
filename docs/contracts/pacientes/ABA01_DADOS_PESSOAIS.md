...
- Cards/seções:
  - **Foto do paciente (seção lateral)**: exibe avatar via `photo_url` (nome legado; armazenar *path* do Storage).
...
|------|-------------------|------------------------|--------|--------|-------------|---------|-----------|--------|----------------|
| Foto do paciente | Foto do paciente | `public.patients.photo_url` | `text` | `string \| null` | Não | `NULL` | nome legado; armazenar *path* do Storage; trim; não permitir string vazia | — | Foto do paciente (path interno). |
...
| Documentos & verificação | Status do CPF | `public.patients.cpf_status` | `text` | `'valid' \| 'invalid' \| 'unknown' \| null` | Não | `valid` | `in ('valid','invalid','unknown')` | select | Status de validação do CPF. |
...
| Naturalidade & filiação | Local de nascimento | `public.patients.place_of_birth` | `text` | `string \| null` | Não | `NULL` | trim; max 200 | — | Local de nascimento (campo livre). |
| Naturalidade & filiação | Cidade de nascimento | `public.patients.place_of_birth_city` | `text` | `string \| null` | Não | `NULL` | trim; max 120 | — | Cidade de nascimento. |
| Naturalidade & filiação | Estado de nascimento | `public.patients.place_of_birth_state` | `text` | `string \| null` | Não | `NULL` | trim; max 60 | — | Estado de nascimento. |
| Naturalidade & filiação | País de nascimento | `public.patients.place_of_birth_country` | `text` | `string \| null` | Não | `Brasil` | trim; max 60 | — | País de nascimento. |
...
- Adotar **padrão do legado**: nomes/colunas/checks do `schema_current.sql` como base.
...
- `updatePatientDadosPessoais(patientId, payload)` onde `payload` contém **apenas** campos da ABA01:
  ...
  - Naturalidade/filiação: `place_of_birth`, `place_of_birth_city`, `place_of_birth_state`, `place_of_birth_country`,
  ...
  - Foto: `photo_url`, `photo_consent`, `photo_consent_date`
...
- Campo opcional (`pref_contact_method`, single-select) com check legado.
...
- `supabase/migrations/202512130704_pacientes_aba01_dados_pessoais_extend_legado.sql`
  - renomeia colunas para nomes do legado (`photo_url`, `pref_contact_method`, `place_of_birth*`)
  - adiciona colunas/checks do legado (pronouns, gender_identity, education_level, race_color, consentimentos, status etc.)
...
- TODO: definir bucket/policies do Storage para `photo_url` (sem expor URLs públicas indevidas).
