# ADRs — Architecture Decision Records

Esta pasta registra decisões arquiteturais canônicas. Cada ADR deve ser pequeno, objetivo e referenciar fontes relacionadas (contratos, runbooks, código, PRs).

Status possíveis: Proposed | Accepted | Superseded | Deprecated.

## Índice

| ADR | Status | Data | Dono | Link | Impacto |
| --- | --- | --- | --- | --- | --- |
| ADR-001 | Accepted | 2025-12-17 | @RenatoMazzarino | [Contract-Driven Development](./ADR-001-contract-driven-development.md) | Alto |
| ADR-002 | Accepted | 2025-12-17 | @RenatoMazzarino | [Multi-tenant + RLS por design](./ADR-002-multi-tenant-rls.md) | Alto |
| ADR-003 | Accepted | 2025-12-17 | @RenatoMazzarino | [Soft delete como padrão](./ADR-003-soft-delete.md) | Alto |
| ADR-004 | Accepted | 2025-12-17 | @RenatoMazzarino | [UI padrão "Dynamics"](./ADR-004-ui-dynamics-standard.md) | Médio |
| ADR-005 | Accepted | 2025-12-17 | @RenatoMazzarino | [Tenancy via app_metadata no JWT](./ADR-005-tenancy-app-metadata-jwt.md) | Alto |
| ADR-006 | Accepted | 2025-12-17 | @RenatoMazzarino | [Política de e-mail no DB](./ADR-006-email-validation-db-policy.md) | Médio |
| ADR-007 | Accepted | 2025-12-27 | Codex | [GED - Custodia, Watermark, Carimbo do Tempo e Acesso ao Original](./ADR-007-ged-custodia-watermark-time-stamp.md) | Alto |
