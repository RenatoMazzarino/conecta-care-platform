...
export type GeocodeRefreshResult = {
  status: string | null;
...
export async function refreshAddressGeocode(addressId: string, force = false): Promise<GeocodeRefreshResult> {
  ...
  const response = await fetch('/api/pacientes/aba02/geocode', {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    ...
    throw new Error(message || 'Falha ao atualizar geocode');
  }

  return (await response.json()) as GeocodeRefreshResult;
}
